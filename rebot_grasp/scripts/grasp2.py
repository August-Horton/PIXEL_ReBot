"""
GraspNet + yolov11s.pt + 盲抓. YOLO 框定目标, GraspNet 生成抓取, 盲闭合不判空抓.

Usage:
    python scripts/grasp2.py --target-class banana --dry-run
    python scripts/grasp2.py --target-class all
"""
...
BLIND_CLOSE_DURATION = 2.0       # s, 盲抓闭合时长
SOFT_CLOSE_TORQUE   = 1.5       # Nm, 闭合扭矩
FAST_KD_CLOSE       = 0.35      # 降阻尼 → vel≈4.3rad/s → ~1.1s 全行程
...
def _execute_grasp(
    ...
    # 盲闭合: 禁用检测阈值, 跑满时长, 不判空抓
    print(f"[Grasp] Blind close {BLIND_CLOSE_DURATION:.1f}s (tau={SOFT_CLOSE_TORQUE}, kd={FAST_KD_CLOSE})")
    _orig_startup = grasp_driver._startup_dist
    _orig_stall = grasp_driver._stall_vel
    _orig_close_torque = grasp_driver._close_torque
    _orig_kd_close = grasp_driver._kd_close
    grasp_driver._startup_dist = float("inf")
    grasp_driver._stall_vel = 0.0
    grasp_driver._close_torque = grasp_driver._close_sign * SOFT_CLOSE_TORQUE
    grasp_driver._kd_close = FAST_KD_CLOSE
    grasp_driver.grasp(timeout=BLIND_CLOSE_DURATION)
    grasp_driver._startup_dist = _orig_startup
    grasp_driver._stall_vel = _orig_stall
    grasp_driver._close_torque = _orig_close_torque
    grasp_driver._kd_close = _orig_kd_close
...
def main() -> int:
    ...
    cfg.setdefault("yolo", {})["model_name"] = "yolov11s.pt"
