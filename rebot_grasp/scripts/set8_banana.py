...
Workflow:
  1. Detect banana -> visual-servo centre with base joint
...
def main() -> int:
    ...
    window_name = "Set8_banana - Visual Servo Pick Banana"
    ...
    try:
        ...
        while True:
            ...
            if auto_trigger:
                ...
                if snap_color is not None:
                    ...
                    for result in snap_results:
                        for box in result.boxes:
                            if "banana" in str(result.names.get(int(box.cls[0]), "")).lower():
                                found = True
                                break
                    if found:
                        _visual_servo_center(cam, model, yolo_opts, controller, "banana", K, cfg)
                        snap_color, snap_depth = cam.get_frame()
                        if snap_color is not None:
                            snap_results = model.predict(
                                snap_color, verbose=False,
                            ...
                            if snap_target is not None:
                                ...
                                if T_hand_eye is not None:
                                    ...
                                    last_display = None
                                    continue
                    print("[AUTO] No more bananas, returning to manual\n")
                    auto_mode = False
                    auto_trigger = False
                    frozen = False
                    last_display = None
            ...
            if key in (ord("g"), ord("G")):
                ...
                print("[Step1] Detect banana for visual servoing")
                snap_color, snap_depth = cam.get_frame()
                ...
                print(f"\n[G] Banana: class={snap_target.class_name} conf={snap_target.conf:.3f}")
