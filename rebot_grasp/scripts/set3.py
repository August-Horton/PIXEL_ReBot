"""
Pick bottle and place at fixed joint angles behind the arm.

放置阶段使用固定关节角 (minimum-jerk 关节空间轨迹), 不做 IK.

Workflow:
  1. Detect bottle via YOLO
  2. Go to pregrasp -> grasp bottle -> lift to ready position
  3. Joint-space move to pre-defined joint angles (behind arm, above box)
...
Usage:
    python scripts/set3.py
    python scripts/set3.py --target bottle
    python scripts/set3.py --dry-run
"""
...
# ============================================================
# 固定放置关节角 (弧度制, joint1 ~ joint6)
# 用 DMTool 把机械臂摆到身后箱子上方, 记录这 6 个数值
# ============================================================
PLACE_JOINTS = (-2.61, -0.57, -0.81, 0.98, 0.0, 0.0)
...
def _move_to_joints(
    ...
    Replicates the safe_home() pattern but targets arbitrary joint angles.
    """
    n = controller._n
...
def _execute_pick_fixed_place(
    ...
    grasp6d: tuple[float, ...],
    pre6d: tuple[float, ...],
    place_joints: tuple[float, ...],
    ready_cfg: dict[str, Any],
    dry_run: bool,
    target_name: str,
) -> bool:
    """Pick target, joint-space move to place, release, return to ready."""
    ...
    # --- 放置阶段: 关节空间轨迹 ---
    print(f"[Step4] Joint-space move to place joints: {[f'{v:+.3f}' for v in place_joints]}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pick target and place at fixed joint angles")
...
def main() -> int:
    ...
    print(f"=== Place joints: {[f'{v:+.3f}' for v in PLACE_JOINTS]} ===")
    cam = make_camera(cfg)
    ...
    window_name = f"Set3 - Pick {target_name} -> Joint Place"
