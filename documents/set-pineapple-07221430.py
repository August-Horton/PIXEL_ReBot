...
def _execute_grasp_sequence(
    ...
    # 步骤 3: Cartesian 抬升 Z 到 0.25, 再调整关节 2/3
    print("[Step3] Cartesian lift Z to 0.25")
    if not controller.move_to_traj(xg, yg, 0.25, rxg, ryg, rzg, duration=2.0):
        print("[Step3] Cartesian lift IK failed")
        return False
    _wait_motion(controller, 2.0)

    print("[Step3] Set joints 2/3 to lift position")
    controller._vlim_override = np.array([0.5, 0.5, 0.5, 0.3, 0.3, 0.3])
...
def _render_display(
    ...
    if best_pineapple is not None:
        ...
        cv2.putText(display, f"pineapple: xyz=({x_m:+.3f},{y_m:+.3f},{z_m:+.3f})",
