...
[WallAvoid] 本文件为 set-cube-07261120.py 的副本, 集成了"凸出墙壁碰撞规避"接入示例。

接入点 (标记 [WallAvoid]):
  1. main() 中初始化 WallAvoidancePlanner
  2. 抓取线程: 视觉伺服居中后, 用深度帧更新环境场景 (墙/凸出物/目标)
  3. _execute_grasp_sequence: 预抓取位/抓取位两段移动改为 _move_safe()
     → 路径规划(直线/绕行) + 逐路点执行 + 动态监测(CRITICAL 触发应急)
...
# [WallAvoid] 避障算法独立模块 (与 wall_collision_avoidance.py 同目录)
from wall_collision_avoidance import (
    Pose6d,
    WallAvoidanceConfig,
    WallAvoidancePlanner,
    depth_to_cloud,
)
...
# [WallAvoid] 带避障的行径封装:
#   路径规划 (直线优先 → 抬升/侧偏绕行) → 逐路点 move_to_traj → 动态监测
def _move_safe(
    controller: RebotArmEndPose,
    planner: Optional[WallAvoidancePlanner],
    tcp_pose_fn: Optional[Callable[[], np.ndarray]],
    x: float,
    y: float,
    z: float,
    roll: float,
    ...
    """带避障的行径。planner 为 None 或场景为空时退化为直接 move_to_traj。"""
    if planner is None or planner.scene.empty():
        return controller.move_to_traj(x, y, z, roll, pitch, yaw, duration=duration)

    # 当前 TCP 位置 (基座系) 作为路径起点
    tcp_pos = tcp_pose_fn()[:3, 3] if tcp_pose_fn is not None else np.array([x, y, z])
    start = Pose6d.from_array(tcp_pos, (roll, pitch, yaw))
    target = Pose6d(x=x, y=y, z=z, roll=roll, pitch=pitch, yaw=yaw)

    path = planner.plan_safe_path(start, target)
    if path is None:
        print(f"[WallAvoid] {label} 无安全路径, 触发应急")
        planner.handle_emergency(f"{label} plan_failed")
        return False
    print(f"[WallAvoid] {label} strategy={path.strategy} "
          f"clearance={path.clearance:.3f}m waypoints={len(path.waypoints)}")

    for i, wp in enumerate(path.waypoints):
        if not controller.move_to_traj(wp.x, wp.y, wp.z, wp.roll, wp.pitch, wp.yaw,
                                       duration=duration):
            print(f"[WallAvoid] {label} waypoint[{i}] IK 失败")
            return False
        _wait_motion(controller, duration)
        # 动态监测 (速度外推按零速度保守估计, 实际可用编码器差分)
        adv = planner.monitor_and_adjust(wp.pos, np.zeros(3))
        if adv.level == "CRITICAL":
            print(f"[WallAvoid] {label} CRITICAL clearance={adv.clearance:.3f}m, 触发应急")
            planner.handle_emergency(f"{label} critical")
            return False
        if adv.level == "ADJUSTING":
            print(f"[WallAvoid] {label} ADJUSTING clearance={adv.clearance:.3f}m")
    return True
...
def _execute_grasp_sequence(
    ...
    dry_run: bool,
    # [WallAvoid] 新增参数: 避障规划器与 TCP 位姿回调
    planner: Optional[WallAvoidancePlanner] = None,
    tcp_pose_fn: Optional[Callable[[], np.ndarray]] = None,
) -> bool:
    ...
    # [WallAvoid] 替换原 move_to_traj: 路径规划 + 动态监测
    if not _move_safe(controller, planner, tcp_pose_fn,
                      xp, yp, zp, rxp, ryp, rzp, 1.5, "pregrasp"):
    ...
    # [WallAvoid] 替换原 move_to_traj: 路径规划 + 动态监测
    if not _move_safe(controller, planner, tcp_pose_fn,
                      xg, yg, zg, rxg, ryg, rzg, 1.0, "grasp"):
...
def main() -> int:
    ...
    # [WallAvoid] 初始化避障规划器 (可通过配置节 wall_avoidance.enable=false 关闭)
    planner: Optional[WallAvoidancePlanner] = None
    wall_cfg = WallAvoidanceConfig.from_dict(cfg.get("wall_avoidance", {}))
    if cfg.get("wall_avoidance", {}).get("enable", True):
        planner = WallAvoidancePlanner(wall_cfg)
        print(f"[WallAvoid] planner enabled: clearance={wall_cfg.path_min_clearance_m}m "
              f"lift={wall_cfg.lift_offset_m}m")
    ...
    try:
        ...
        def _auto_process_loop() -> None:
            ...
            while True:
                ...
                # [WallAvoid] 更新环境场景: 深度帧 → 点云 → 墙面/凸出物
                if planner is not None:
                    try:
                        cloud = depth_to_cloud(
                            snap_depth, K,
                            planner.cfg.min_depth_m, planner.cfg.max_depth_m)
                        planner.update_scene_from_cloud(
                            cloud, T_cam2base, target_cam=snap_cube.position)
                        print(f"[WallAvoid] scene updated: walls={len(planner.scene.walls)} "
                              f"obstacles={len(planner.scene.obstacles)}")
                    except Exception as exc:
                        print(f"[WallAvoid] scene update failed: {exc}")
                ...
                ok = _execute_grasp_sequence(
                    ...
                    # [WallAvoid] 传入避障规划器与 TCP 位姿回调
                    planner=planner,
                    tcp_pose_fn=lambda: grasp_driver.get_tcp_pose(),
                )
