...
Keys:
  G: capture, grasp, and auto-place bottle
            ...
            while True:
                ...
                for grasp in snap_grasps:
                    ...
                    if "water bottle" in grasp.class_name.lower():
                        if snap_bottle is None or grasp.bbox_xyxy[0] < snap_bottle.bbox_xyxy[0]:
                            snap_bottle = grasp

                if snap_bottle is None:
                    print(f"[G] No bottle found, waiting... (processed {bottle_count})")
                    time.sleep(1.0)
                    continue

                bottle_count += 1
                print(f"\n[G] === Bottle #{bottle_count}: class={snap_bottle.class_name} conf={snap_bottle.conf:.3f}")
                print(f"  position_xyz={snap_bottle.position.tolist()}")

                bottle_cam_x = float(snap_bottle.position[0])
                side = "right" if bottle_cam_x > 0 else "left"
                print(f"  cam_x={bottle_cam_x:+.3f} -> {side} side")
                ...
                if not ok:
                    print(f"[G] ERROR: bottle #{bottle_count} grasp failed (IK or empty), returning to ready")
                    print(f"[G] Joints before reset: {rebotarm.arm.get_positions().tolist()}")
                    _move_ready(controller, ready_cfg)
                    continue

                print(f"[G] Bottle #{bottle_count} grasped!")

                # ---- 放置: 旋转 joint1 ----
                rotate_q = rebotarm.arm.get_positions()
                print(f"[G] Joints after lift, before rotate: {rotate_q.tolist()}")
                rotate_q[0] = -2.61 if bottle_cam_x > 0 else 2.61
                ...
                print("[G] Open gripper to release bottle")
                ...
                print(f"[G] Bottle #{bottle_count} placed!")

            # 模式关闭, 复位
            print(f"[G] Grasp mode off, processed {bottle_count} bottles")
        ...
        while True:
            ...
            if key in (ord("g"), ord("G")):
                ...
                if grasp_mode:
                    ...
                    print("\n[G] === Grasp mode OFF (resetting after current bottle) ===")
