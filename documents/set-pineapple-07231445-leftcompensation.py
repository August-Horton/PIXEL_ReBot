"""
Grasp pineapple demo based on YOLO.

Workflow:
  1. Detect pineapple
  2. Go to grasp pineapple
  3. Grasp pineapple and lift to ready position
  4. Auto: move arm to placement joints (left/right), open gripper to release

Keys:
  G: capture, grasp, and auto-place pineapple
...
def _execute_grasp_sequence(
    ...
    print("[Step3] Cartesian lift Z to 0.15")
    if not controller.move_to_traj(xg, yg, 0.15, rxg, ryg, rzg, duration=2.0):
...
def _render_display(
    ...
    best_pineapple: Optional[GraspPose],
    status_text: str,
) -> np.ndarray:
    """在图像上叠加 YOLO 检测框、OBB方向、状态文本和最优菠萝位置信息。"""
    display = image.copy()

    # 绘制所有检测到的物体的边界框与类别/置信度
    for grasp in grasps:
        is_pineapple = "pineapple" in grasp.class_name.lower()
        color = (0, 255, 0) if grasp.is_valid else (0, 165, 255)

        # OBB 方向可视化 + 长轴/短轴 (仅菠萝)
        if is_pineapple and grasp.is_valid:
    ...
    # 底部显示最优菠萝的相机坐标系 xyz + OBB角度
    if best_pineapple is not None:
        x_m, y_m, z_m = best_pineapple.position.tolist()
        long_deg = best_pineapple.long_axis_angle_deg
        short_deg = best_pineapple.angle_deg
        cv2.putText(display,
...
def parse_args() -> argparse.Namespace:
    """解析命令行参数。"""
    parser = argparse.ArgumentParser(description="Grasp pineapple and position arm demo")
...
def main() -> int:
    ...
    # ---- OpenCV 窗口 ----
    window_name = "Set - Grasp Pineapple"
    ...
    try:
            ...
            while True:
                ...
                for grasp in snap_grasps:
                    if not grasp.is_valid:
                        continue
                    if "pineapple" in grasp.class_name.lower():
                        if snap_pineapple is None or grasp.bbox_xyxy[0] < snap_pineapple.bbox_xyxy[0]:
                            snap_pineapple = grasp

                if snap_pineapple is None:
                    print(f"[G] No pineapple found, waiting... (processed {pineapple_count})")
                    time.sleep(1.0)
                    continue

                pineapple_count += 1
                print(f"\n[G] === Pineapple #{pineapple_count}: class={snap_pineapple.class_name} conf={snap_pineapple.conf:.3f}")
                print(f"  position_xyz={snap_pineapple.position.tolist()}")

                pineapple_cam_x = float(snap_pineapple.position[0])
                side = "right" if pineapple_cam_x > 0 else "left"
                print(f"  cam_x={pineapple_cam_x:+.3f} -> {side} side")
                ...
                grasp6d, pre6d = transform_grasp_pose_to_base(
                    snap_pineapple.position,
                    snap_pineapple.tcp_rotation,
                    T_cam2base,
                    pregrasp_offset_m,
                )

                # 左侧补偿: 菠萝cam_x超过中心线左侧0.01时，左移0.04 + 减小抓取深度0.05
                if pineapple_cam_x < -0.01:
                    print(f"  Left compensation: cam_x={pineapple_cam_x:+.3f} < -0.01, shift left 0.04, reduce depth 0.05")
                    grasp6d = (grasp6d[0] - 0.02, grasp6d[1]+0.03, grasp6d[2] - 0.05, grasp6d[3], grasp6d[4], grasp6d[5])
                    pre6d = (pre6d[0] - 0.02, pre6d[1]+0.03, pre6d[2] - 0.05, pre6d[3], pre6d[4], pre6d[5])
                ...
                if not ok:
                    print(f"[G] ERROR: pineapple #{pineapple_count} grasp failed (IK or empty), returning to ready")
                    print(f"[G] Joints before reset: {rebotarm.arm.get_positions().tolist()}")
                    _move_ready(controller, ready_cfg)
                    continue

                print(f"[G] Pineapple #{pineapple_count} grasped!")

                # ---- 放置: 旋转 joint1 ----
                rotate_q = rebotarm.arm.get_positions()
                print(f"[G] Joints after lift, before rotate: {rotate_q.tolist()}")
                rotate_q[0] = -2.61 if pineapple_cam_x > 0 else 2.61
                ...
                print("[G] Open gripper to release pineapple")
                ...
                print(f"[G] Pineapple #{pineapple_count} placed!")

            # 模式关闭, 复位
            print(f"[G] Grasp mode off, processed {pineapple_count} pineapples")
        ...
        while True:
            ...
            # ---- 从检测结果中找出置信度最高的 pineapple ----
            best_pineapple = None
            for grasp in last_grasps:
                if not grasp.is_valid:
                    continue
                if "pineapple" in grasp.class_name.lower():
                    if best_pineapple is None or grasp.conf > best_pineapple.conf:
                        best_pineapple = grasp

            # ---- 状态栏文本 ----
            status = f"LIVE  {fps_value:.1f}fps | G=grasp+place R=resume Q=quit"

            # ---- 渲染并显示 ----
            display = _render_display(color_bgr, last_grasps, best_pineapple, status)
            ...
            if key in (ord("g"), ord("G")):
                ...
                if grasp_mode:
                    ...
                    print("\n[G] === Grasp mode OFF (resetting after current pineapple) ===")
